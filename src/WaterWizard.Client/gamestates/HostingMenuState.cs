...
public class HostingMenuState : IGameState
{
    ...
    private void DrawHostMenu(GameStateManager manager)
    {
        ...
        Raylib.DrawText($"Hosting on: {publicIp}:{hostPort}", (manager.screenWidth - titleWidth) / 2, manager.screenHeight / 3, 20, Color.DarkGreen);
        string statusText = isPlayerConnected ? "Player Connected!" : "Waiting for players...";
        int statusWidth = Raylib.MeasureText(statusText, 20);
        Raylib.DrawText(statusText, (manager.screenWidth - statusWidth) / 2, manager.screenHeight / 3 + 40, 20, isPlayerConnected ? Color.Green : Color.DarkBlue);
        int margin = 20;
        int buttonHeight = 40;
        int buttonWidth = 120;
