...
public static class RockFactory
{
    ...
    )
    {
        if (
            ...
            && IsValidRockPlacement(board, position.X, position.Y, rockPositions)
        )
        {
    ...
    /// </summary>
    private static List<(int X, int Y)> GetPositionsInZone(
        Rectangle zone,
        HashSet<(int X, int Y)> availablePositions
    )
    {
    ...
    /// </summary>
    private static bool IsValidRockPlacement(
        Cell[,] board,
        int x,
        int y,
        List<(int X, int Y)> existingRocks
    )
    {
