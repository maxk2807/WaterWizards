...
public class Program
{
    public static async Task<int> Main(string[] args)
    {
        ...
        try
        {
            ...
            Console.WriteLine(
                $"✅ Analytics erfolgreich generiert in {(endTime - startTime).TotalMilliseconds:F0}ms"
            );
        ...
        try
        {
            ...
            if (analytics != null)
            {
                ...
                if (analytics.GitStatistics != null)
                {
                    ...
                    Console.WriteLine(
                        $"👤 Letzter Autor: {analytics.GitStatistics.LastCommitAuthor}"
                    );
                }
                ...
                if (analytics.CodeStatistics != null)
                {
                    ...
                    Console.WriteLine(
                        $"💾 Größe: {analytics.CodeStatistics.TotalSize / 1024.0:F1} KB"
                    );
                }

                if (analytics.DeveloperStatistics != null && analytics.DeveloperStatistics.Any())
                {
                    var topDeveloper = analytics
                        .DeveloperStatistics.OrderByDescending(d => d.TotalCommits)
                        .First();
                    Console.WriteLine(
                        $"👑 Top Entwickler: {topDeveloper.Name} ({topDeveloper.TotalCommits:N0} Commits)"
                    );
                }
                ...
                {
                    Console.WriteLine(
                        $"🎯 Code/Comment Ratio: {analytics.QualityMetrics.CodeToCommentRatio:F1}"
                    );
                    Console.WriteLine(
                        $"📊 Durchschnittliche Zeilen pro Datei: {analytics.QualityMetrics.AverageLinesPerFile:F0}"
                    );
                }
            }
        }
