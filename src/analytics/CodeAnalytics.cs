...
public static class CodeAnalytics
{
    ...
    public static async Task<string> GenerateRepositoryAnalytics(string repositoryPath = ".")
    {
        ...
        try
        {
            ...
            var jsonReport = JsonSerializer.Serialize(
                analytics,
        ...
        catch (Exception ex)
        {
            ...
            return JsonSerializer.Serialize(
                new { Error = ex.Message, GeneratedAt = DateTime.UtcNow }
            );
        }
    }
    ...
    private static string GenerateReport(RepositoryAnalytics analytics)
    {
        ...
        if (analytics.GitStatistics != null)
        {
            ...
            report.AppendLine(
                $"- **Uncommitted Changes**: {analytics.GitStatistics.UncommittedChanges}"
            );
            report.AppendLine($"- **Last Commit**: {analytics.GitStatistics.LastCommitMessage}");
            report.AppendLine(
                $"- **Last Commit Author**: {analytics.GitStatistics.LastCommitAuthor}"
            );
            report.AppendLine(
                $"- **Last Commit Date**: {analytics.GitStatistics.LastCommitDate:yyyy-MM-dd HH:mm:ss}"
            );
            report.AppendLine();
        }
        ...
        if (analytics.DeveloperStatistics != null && analytics.DeveloperStatistics.Any())
        {
            ...
            foreach (
                var dev in analytics.DeveloperStatistics.OrderByDescending(d => d.TotalCommits)
            )
            {
                ...
                report.AppendLine(
                    $"- **Feature Commits**: {dev.FeatureCommits:N0} ({dev.FeatureCommitPercentage:F1}%)"
                );
                report.AppendLine(
                    $"- **Bug Fix Commits**: {dev.BugFixCommits:N0} ({dev.BugFixCommitPercentage:F1}%)"
                );
                report.AppendLine(
                    $"- **Refactor Commits**: {dev.RefactorCommits:N0} ({dev.RefactorCommitPercentage:F1}%)"
                );
                report.AppendLine(
                    $"- **Documentation Commits**: {dev.DocumentationCommits:N0} ({dev.DocumentationCommitPercentage:F1}%)"
                );
                report.AppendLine();
                ...
                report.AppendLine(
                    $"- **Most Active Week**: {dev.MostActiveWeek} ({dev.MaxCommitsInWeek} commits)"
                );
                report.AppendLine(
                    $"- **Most Active Month**: {dev.MostActiveMonth} ({dev.MaxCommitsInMonth} commits)"
                );
                report.AppendLine();
            }
        }
        ...
        if (analytics.QualityMetrics != null)
        {
            ...
            report.AppendLine(
                $"- **Average Methods Per Class**: {metrics.AverageMethodsPerClass:F1}"
            );
            report.AppendLine(
                $"- **Average Properties Per Class**: {metrics.AveragePropertiesPerClass:F1}"
            );
            report.AppendLine($"- **Code Complexity**: {metrics.CodeComplexity:F1} lines/method");
            report.AppendLine(
                $"- **Documentation Coverage**: {metrics.DocumentationCoverage:F1} lines/class"
            );
            report.AppendLine();
            ...
            report.AppendLine(
                $"- **Average Commits Per Developer**: {metrics.AverageCommitsPerDeveloper:F1}"
            );
            report.AppendLine();
            ...
            report.AppendLine(
                $"- **{metrics.TotalDevelopers} talented developers** working together"
            );
            report.AppendLine(
                $"- **{metrics.ProjectVelocity:F1} commits per day** average velocity"
            );
            report.AppendLine(
                $"- **{metrics.AverageCommitsPerDeveloper:F1} commits per developer** showing dedication"
            );
            report.AppendLine(
                $"- **{metrics.CodeToCommentRatio:F1} code-to-comment ratio** indicating good documentation"
            );
            report.AppendLine(
                $"- **{metrics.EmptyLinesPercentage:F1}% empty lines** showing clean, readable code"
            );
            report.AppendLine();
            ...
            if (analytics.DeveloperStatistics != null && analytics.DeveloperStatistics.Any())
            {
                var topDevelopers = analytics
                    .DeveloperStatistics.OrderByDescending(d => d.TotalCommits)
                    .Take(3)
                    .ToList();
                ...
                for (int i = 0; i < topDevelopers.Count; i++)
                {
                    ...
                    var medal =
                        ...
                        : "🥉";
                    report.AppendLine(
                        $"- {medal} **{dev.Name}**: {dev.TotalCommits} commits ({dev.FeatureCommits} features, {dev.BugFixCommits} bugfixes)"
                    );
                }
                report.AppendLine();
            }
    ...
    /// </summary>
    private static async Task SaveReport(
    ...
    private static async Task CleanupOldReports(string analyticsDir, int maxReports)
    {
        try
        {
            var jsonFiles = Directory
                .GetFiles(analyticsDir, "analytics_*.json")
                .Where(f => !f.EndsWith("latest.json"))
                .OrderByDescending(f => f)
                .ToList();

            var markdownFiles = Directory
                .GetFiles(analyticsDir, "analytics_*.md")
            ...
            foreach (var file in filesToDelete)
            {
                ...
                {
                    Console.WriteLine(
                        $"[CodeAnalytics] Konnte Datei nicht löschen: {file} - {ex.Message}"
                    );
                }
            }
