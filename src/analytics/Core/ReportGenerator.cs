...
public class ReportGenerator
{
    ...
    private string GenerateFullReport()
    {
        ...
        if (_analytics.GitStatistics != null)
        {
            ...
            report.AppendLine(
                $"- **Uncommitted Changes**: {_analytics.GitStatistics.UncommittedChanges}"
            );
            report.AppendLine($"- **Last Commit**: {_analytics.GitStatistics.LastCommitMessage}");
            report.AppendLine(
                $"- **Last Commit Author**: {_analytics.GitStatistics.LastCommitAuthor}"
            );
            report.AppendLine(
                $"- **Last Commit Date**: {_analytics.GitStatistics.LastCommitDate:yyyy-MM-dd HH:mm:ss}"
            );
            report.AppendLine();
        }
        ...
        if (_analytics.DeveloperStatistics != null && _analytics.DeveloperStatistics.Any())
        {
            ...
            foreach (
                var dev in _analytics.DeveloperStatistics.OrderByDescending(d => d.TotalCommits)
            )
            {
